...
let ticketPriceTotal = runners.reduce(function (accumulator, items) {
...
// Problem 1
// In order to contact each Runners place of employment we want a list of every Company and their email  address.
let companyNames = [];

runners.forEach(function (items) {
  return companyNames.push(`${items.company_name}` + " | " + `${items.email}`);
});
console.log(companyNames.sort());

// Problem 2
// In order to print names for the back of runners shirts, we want all last names in uppercase in a new array.
...
// Problem 3
//In order to know who to attempt to get more money from, create a list of people who clearly have more money..A.K.A, anyone who can donate more than $250 to a race.
let donationSize = [
  runners.filter(function (items) {
    return items.donation >= "250";
  })
];
